...
import {
  DashboardOutlined, FileTextOutlined, BarChartOutlined, UserOutlined,
  ...
  LineChartOutlined, FilePdfOutlined,
...
const menuItems: MenuItem[] = [
  { key: '/observatoire/dashboard', icon: <DashboardOutlined />, label: 'Dashboard' },
  { key: '/observatoire/analytics/academic', icon: <LineChartOutlined />, label: 'Academic Analytics' },
  { key: '/observatoire/analytics/insertion', icon: <BarChartOutlined />, label: 'Insertion Analytics' },
  { key: '/observatoire/surveys', icon: <FileTextOutlined />, label: 'Surveys' },
  { key: '/observatoire/charts', icon: <FundProjectionScreenOutlined />, label: 'Charts' },
  { key: '/observatoire/reports', icon: <FilePdfOutlined />, label: 'Reports' },
  { key: '/observatoire/profile', icon: <UserOutlined />, label: 'Profile' },
];

const breadcrumbMap: Record<string, string> = {
  '/observatoire/dashboard': 'Dashboard',
  '/observatoire/analytics/academic': 'Academic Analytics',
  '/observatoire/analytics/insertion': 'Insertion Analytics',
  '/observatoire/surveys': 'Surveys',
  '/observatoire/surveys/new': 'New Survey',
  '/observatoire/charts': 'Charts',
  '/observatoire/reports': 'Reports',
  '/observatoire/profile': 'Profile',
};

export function ObservatoireLayout() {
  ...
  const selectedKey = Object.keys(breadcrumbMap).find((key) => location.pathname.startsWith(key)) || location.pathname;
        ...
        <div style={{
          ...
          {!collapsed && (
            <div>
              ...
              <Text style={{ fontSize: 11, color: token.colorTextTertiary, whiteSpace: 'nowrap', textTransform: 'uppercase', letterSpacing: 0.5 }}>Observatoire</Text>
            </div>
          )}
        </div>
        <Menu
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            ...
            <Dropdown menu={{ items: userMenuItems }} placement="bottomRight" trigger={['click']}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', padding: '6px 12px', borderRadius: 10, background: token.colorBgSpotlight, transition: 'all 0.2s' }}>
                <Avatar size={32} style={{ background: 'linear-gradient(135deg, #f59e0b, #ef4444)', fontWeight: 600 }} icon={<UserOutlined />} />
                {user && (
                  <div style={{ lineHeight: 1.2 }}>
                    <Text style={{ fontSize: 13, fontWeight: 600, color: token.colorText, display: 'block' }}>{user.firstName} {user.lastName}</Text>
                    <Text style={{ fontSize: 11, color: token.colorTextTertiary, textTransform: 'capitalize' }}>Responsable</Text>
                  </div>
                )}
              </div>
            </Dropdown>
