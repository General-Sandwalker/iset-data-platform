...
const menuItems: MenuItem[] = [
  ...
  { key: '/teacher/dashboards', icon: <BarChartOutlined />, label: 'Academic Dashboards' },
  { key: '/teacher/surveys', icon: <FileTextOutlined />, label: 'Surveys' },
  { key: '/teacher/profile', icon: <UserOutlined />, label: 'Profile' },
];

const breadcrumbMap: Record<string, string> = {
  '/teacher/dashboard': 'Dashboard',
  '/teacher/dashboards': 'Academic Dashboards',
        ...
        <div style={{
          ...
          {!collapsed && (
            <div>
              ...
              <Text style={{ fontSize: 11, color: token.colorTextTertiary, whiteSpace: 'nowrap', textTransform: 'uppercase', letterSpacing: 0.5 }}>Teacher Portal</Text>
            </div>
          )}
        </div>
        <Menu
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', padding: '6px 12px', borderRadius: 10, background: token.colorBgSpotlight, transition: 'all 0.2s' }}>
                ...
                {user && (
                  <div style={{ lineHeight: 1.2 }}>
                    ...
                    <Text style={{ fontSize: 11, color: token.colorTextTertiary, textTransform: 'capitalize' }}>Teacher</Text>
                  </div>
                )}
              </div>
            </Dropdown>
