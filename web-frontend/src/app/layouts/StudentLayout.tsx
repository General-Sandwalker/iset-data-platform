...
import {
  DashboardOutlined, FileTextOutlined, DatabaseOutlined, UserOutlined,
...
const menuItems: MenuItem[] = [
  ...
  { key: '/student/surveys', icon: <FileTextOutlined />, label: 'Surveys' },
  { key: '/student/my-data', icon: <DatabaseOutlined />, label: 'My Data' },
  { key: '/student/dashboards', icon: <FundProjectionScreenOutlined />, label: 'Dashboards' },
...
const breadcrumbMap: Record<string, string> = {
  ...
  '/student/my-data': 'My Data',
  '/student/dashboards': 'Dashboards',
        ...
        <div style={{
          ...
          {!collapsed && (
            <div>
              ...
              <Text style={{ fontSize: 11, color: token.colorTextTertiary, whiteSpace: 'nowrap', textTransform: 'uppercase', letterSpacing: 0.5 }}>Student Portal</Text>
            </div>
          )}
        </div>
        <Menu
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            ...
            <Dropdown menu={{ items: userMenuItems }} placement="bottomRight" trigger={['click']}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', padding: '6px 12px', borderRadius: 10, background: token.colorBgSpotlight, transition: 'all 0.2s' }}>
                <Avatar size={32} style={{ background: 'linear-gradient(135deg, #6366f1, #22d3ee)', fontWeight: 600 }} icon={<UserOutlined />} />
                {user && (
                  <div style={{ lineHeight: 1.2 }}>
                    <Text style={{ fontSize: 13, fontWeight: 600, color: token.colorText, display: 'block' }}>{user.firstName} {user.lastName}</Text>
                    <Text style={{ fontSize: 11, color: token.colorTextTertiary, textTransform: 'capitalize' }}>Student</Text>
                  </div>
                )}
              </div>
            </Dropdown>
