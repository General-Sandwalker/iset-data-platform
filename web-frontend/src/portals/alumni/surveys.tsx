  ...
  return (
    <div>
      <div style={{ marginBottom: 28 }}>
        <Title level={3} style={{ marginBottom: 4, color: token.colorText, fontWeight: 700 }}>Insertion Surveys</Title>
        <Text style={{ color: token.colorTextSecondary }}>Complete surveys about your professional situation</Text>
      </div>
      ...
      <Spin spinning={loading}>
        {surveys.length > 0 ? (
                ...
                >
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 'auto' }}>
                    ...
                    {survey.published_slug && (
                      <Button type="primary" size="small" icon={<ArrowRightOutlined />} onClick={() => navigate(`/alumni/surveys/${survey.published_slug}`)}>
                        Take Survey
                      </Button>
                    )}
                  </div>
          ...
          <Empty description="No insertion surveys available" style={{ padding: 60 }} />
        )}
      </Spin>
    </div>
  );
