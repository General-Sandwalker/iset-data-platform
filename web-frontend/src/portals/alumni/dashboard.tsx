...
export default function AlumniDashboard() {
  ...
  const loadStats = useCallback(async () => {
    try {
      const res = await apiClient.get<ApiResponse<AlumniStats>>('/public/alumni-stats');
  ...
  const statCards = [
    { title: 'Available Surveys', value: stats?.availableSurveys ?? 0, icon: <FileTextOutlined />, color: '#a855f7' },
    { title: 'My Records', value: stats?.myRecords ?? 0, icon: <DatabaseOutlined />, color: '#10b981' },
    { title: 'Insertion Dashboards', value: stats?.publishedDashboards ?? 0, icon: <BarChartOutlined />, color: '#6366f1' },
  ];
  ...
  return (
    <div>
      <div style={{ marginBottom: 28 }}>
        ...
        <Text style={{ color: token.colorTextSecondary }}>Alumni Portal Dashboard</Text>
      </div>
      ...
      <Card style={{ marginTop: 24, borderRadius: 16, border: `1px solid ${token.colorBorderSecondary}` }}>
        ...
        <Space wrap>
          <a onClick={() => window.location.hash = '/alumni/profile'} style={{ color: token.colorPrimary }}>Update Professional Profile</a>
          <a onClick={() => window.location.hash = '/alumni/surveys'} style={{ color: token.colorPrimary }}>Complete Surveys</a>
          <a onClick={() => window.location.hash = '/alumni/dashboards'} style={{ color: token.colorPrimary }}>View Insertion Stats</a>
        </Space>
      </Card>
    </div>
  );
